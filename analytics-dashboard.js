...
function renderChart() {
    ...
    $('chart').innerHTML = `<div class="chart-readout" id="chartReadout" aria-live="polite"></div><svg class="chart-plot" id="chartPlot" viewBox="0 0 500 160" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="chartFill" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#b5db9f" stop-opacity=".18"/><stop offset="1" stop-color="#b5db9f" stop-opacity="0"/></linearGradient></defs><path d="M0 20H500M0 82H500M0 145H500" stroke="#344036" stroke-dasharray="3 5" fill="none"/><path d="${path}L${points.at(-1).x},160L${points[0].x},160Z" fill="url(#chartFill)"/><path d="${path}" fill="none" stroke="#b5db9f" stroke-width="2.5" vector-effect="non-scaling-stroke"/><circle id="chartDot" r="4" fill="#f1eee4" stroke="#1a201b" stroke-width="2"/></svg><div class="chart-axis"><span>${esc(chartDays[0].label)}</span><span>${max} max / day</span><span>${esc(chartDays.at(-1).label)}</span></div><label class="note chart-helper" for="chartSlider">${chartDays.length>1?'Slide to inspect a day':'Today\u2019s recorded visitors'}</label><input class="chart-slider" id="chartSlider" type="range" min="0" max="${chartDays.length-1}" value="${chartDays.length-1}" step="1" ${chartDays.length===1?'disabled':''}>`;
...
function renderVisits() {
    $('visitCount').textContent = `${number(totalVisits)} ${totalVisits===1?'visit':'visits'}`;
    $('visits').innerHTML = sessions.length ? sessions.map(s=>{
        const path = [...new Set((s.path||[]).map(clipName))];
        const route = path.length ? path.slice(0,2).join(' \u2192 ')+(path.length>2?` +${path.length-2} more`:'') : 'No project opened';
        return `<button class="visit-card" data-session="${esc(s.id)}" aria-haspopup="dialog"><span class="visit-heading"><strong>${s.guest_number?`Guest ${esc(s.guest_number)}`:'Visitor'}</strong>${visitStatus(s)}</span><span class="visit-meta">${esc(stamp(s.started_at))} &middot; ${esc(sessionDuration(s))}</span><span class="visit-path">${esc(route)}</span><span class="visit-footer"><span>${esc([s.os||s.device,s.browser].filter(Boolean).join(' / ')||'Device unknown')} &middot; ${esc(sourceLabel(s.referrer,s.landing_path))}</span><span class="arrow" aria-hidden="true">&rarr;</span></span></button>`;
    }).join('') : '<div class="empty"><strong>No visits in this view</strong>Try a different date range or device.</div>';
    $('loadMore').hidden = sessions.length>=totalVisits; $('loadMore').disabled = listBusy;
    $('visitScope').textContent = sessions.length<totalVisits ? `${number(sessions.length)} of ${number(totalVisits)} visits shown` : '';
}
...
async function loadBoard() {
    ...
    try {
        const [o,list] = await Promise.all([api(`/analytics/overview?range=${range}&tz=${new Date().getTimezoneOffset()}`,controller.signal),api(`/analytics/sessions?${listParams()}`,controller.signal)]);
        if (controller!==boardController) return false;
        overview=o; sessions=list.sessions||[]; totalVisits=Number(list.total)||0;
        loadedRange=range; loadedDevice=$('deviceFilter').value;
        renderAll(); lastUpdated=Date.now(); $('freshness').textContent=`Updated ${new Date(lastUpdated).toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'})}`;
        return true;
    } catch(e) {
        if(controller!==boardController) return false;
        if (overview) {
...
async function loadVisits(append=false) {
    ...
    try {
        const list=await api(`/analytics/sessions?${listParams(append?sessions.length:0)}`,controller.signal);
        if(controller!==listController)return;
        const old=sessions.length;
        sessions=append?sessions.concat(list.sessions||[]):list.sessions||[]; totalVisits=Number(list.total)||0;
        loadedDevice=$('deviceFilter').value;
        renderVisits(); if(append)$('visits').children[old]?.focus({preventScroll:true});
        $('errorBanner').hidden=true;
    } catch(e) { if(controller===listController){$('deviceFilter').value=loadedDevice;reportError(e.name==='AbortError'?new Error('Loading visits timed out.'):e);} }
...
function renderDetail(detail) {
    const s={...sessions.find(s=>s.id===selectedId),...detail.session};
    ...
    const facts=[['Visit length',sessionDuration(s)],['Recorded watch',watchLabel(seconds)],['Projects opened',number(new Set(events.filter(e=>e.action==='video_view').map(e=>e.payload?.videoId === 'landing-video' ? 'landing' : e.payload?.videoId).filter(Boolean)).size)],['Link clicks',number(clicks)]];
    const tech=[['Device',[s.os||s.device,s.browser].filter(Boolean).join(' / ')||'Unknown'],['Connection',s.connection||snapshot.connection||'Not reported'],['Viewport',s.viewport||snapshot.viewport||'Not reported'],['Timezone',s.timezone||snapshot.timezone||'Not reported'],['Server response',milliseconds(s.ttfb_ms??snapshot.ttfbMs)],['First content paint',milliseconds(s.fcp_ms??snapshot.fcpMs)],['Page load',milliseconds(s.load_ms??snapshot.loadMs)],['Language',s.language||snapshot.language||'Not reported'],['Reduced motion',s.reduced_motion!=null?(s.reduced_motion?'On':'Off'):snapshot.reducedMotion!=null?(snapshot.reducedMotion?'On':'Off'):'Not reported'],['Visit number',s.visit_number||'Not reported']];
    const markup=rows=>`<dl class="facts">${rows.map(([label,value])=>`<div><dt>${esc(label)}</dt><dd class="numeric">${esc(value)}</dd></div>`).join('')}</dl>`;
    $('visitDetail').innerHTML=`<div class="dialog-hero">${visitStatus(s)}<h2>${s.guest_number?`Guest ${esc(s.guest_number)}`:'Visitor'}</h2><p>${esc(stamp(s.started_at))}<br>From ${esc(sourceLabel(s.referrer,s.landing_path))}</p></div>${markup(facts)}<h3>The journey</h3><ol class="timeline">${events.map(e=>`<li><p>${esc(eventText(e))}</p><time>${esc(stamp(e.ts))}</time></li>`).join('')||'<li><p>No events recorded.</p></li>'}</ol><details class="tech-data"><summary>Device & loading details</summary>${markup(tech)}<p>Timing values describe page loading, not video readiness.</p><p>Arrival page: ${esc(s.landing_path||'Not reported')}</p><p>Referrer: ${esc(s.referrer||'Not reported')}</p></details>`;
}
async function fetchDetail(id) {
    detailController?.abort();
    const controller=detailController=new AbortController(), timeout=setTimeout(()=>controller.abort(),20000);
    $('visitDetail').innerHTML='<p class="empty" role="status">Loading this visit...</p>';
    try {const detail=await api(`/analytics/sessions/${encodeURIComponent(id)}`,controller.signal);if(selectedId===id&&controller===detailController)renderDetail(detail);}
    catch(e) {if(controller!==detailController)return;if(e.auth){reportError(e);return;}$('visitDetail').innerHTML='<div class="empty"><strong>Could not load this visit</strong><p>Please try again.</p><button class="soft-button" id="retryDetail">Retry</button></div>';$('retryDetail').onclick=()=>fetchDetail(id);}
...
function openGuest(id) {
    ...
    history.pushState({rfVisit:id},'',location.href); $('closeVisit').focus({preventScroll:true}); fetchDetail(id);
}
function dismissDetail() {
    detailController?.abort(); detailController=null; selectedId='';
